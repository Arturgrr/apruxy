...
var UserCommand = class extends BaseCommand {
...
UserCommand.data = new import_discord2.SlashCommandBuilder().setName("user").setDescription("A bunch of user commands").setDescriptionLocalizations({
  "pt-BR": "Um monte de comandos do usu\xE1rio"
}).addSubcommand(
  (subcommand) => subcommand.setName("info").setDescription("Get user info").setDescriptionLocalizations({
    "pt-BR": "Veja as informa\xE7\xF5es do usu\xE1rio"
  }).addUserOption(
    new import_discord2.SlashCommandUserOption().setName("user").setNameLocalizations({ "pt-BR": "usu\xE1rio" }).setDescription("User to get info").setDescriptionLocalizations({
      "pt-BR": "Usu\xE1rio para ver as informa\xE7\xF5es"
    }).setRequired(false)
  )
);
