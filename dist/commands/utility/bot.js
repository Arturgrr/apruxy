...
BotCommand.data = new import_discord3.SlashCommandBuilder().setName("bot").setDescription("A bunch of bot commands").setDescriptionLocalizations({
  "pt-BR": "Um monte de comandos do bot"
}).addSubcommand(
  (subcommand) => subcommand.setName("ping").setDescription("Replies with pong!").setDescriptionLocalizations({
    "pt-BR": "Responde com pong!"
  })
).addSubcommand(
  (subcommand) => subcommand.setName("info").setDescription("Get bot info").setDescriptionLocalizations({
    "pt-BR": "Veja as informa\xE7\xF5es do bot"
  })
);
